...
# Renombrar la columna 'uf' a 'ciudad'
...
def preprocesamiento():
  ...
  df_itens_pedidos=pd.DataFrame(df_itens_pedidos).reset_index(drop=True)
  df_pedidos=pd.DataFrame(df_pedidos).reset_index(drop=True)
  df_productos=pd.DataFrame(df_productos).reset_index(drop=True)
  df_vendedores=pd.DataFrame(df_vendedores).reset_index(drop=True)
...
# Realizar el merge con df_location usando 'ciudad'
df_final = pd.merge(merged3, df_location['ciudad'], on='ciudad', how='left')
