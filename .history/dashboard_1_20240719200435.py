...
df_ventas = pd.read_csv('df_final_limpio.csv')
df_ventas['valor_total'] = df_ventas['valor_total'].sum()
#df_ventas['order_purchase_timestamp'] = pd.to_datetime(df_ventas['order_purchase_timestamp'])
df_ventas['tipo_producto'] = df_ventas['producto'].str.split('').str[0]
...
estados = sorted(list(df_ventas['Estado'].unique()))
...
#Filtrando los datos
if ciudades:
	df_ventas = df_ventas[df_ventas['Estado'].isin(ciudades)]

if producto!='Todos':
	df_ventas = df_ventas[df_ventas['tipo_producto'] == producto]

if not años:
	df_ventas = df_ventas[df_ventas['fecha_compra'].dt.year == año]
